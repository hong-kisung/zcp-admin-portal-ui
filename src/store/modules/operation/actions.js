...
export default {
	getOrders: function(store, payload) {
		...
		axios.get('api/operation/orders?q=' + q + '&pageNo=' + pageNo + '&pageSize=' + pageSize).then(response => {
	...
	getOrder: function (store, payload) {
		axios.get('api/operation/orders/' + payload.id).then(response => {
	...
	getClusters: function(store, payload) {
		axios.get('api/operation/clusters').then(response => {
