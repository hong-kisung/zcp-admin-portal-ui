...
export default {
	...
	getClusterProjects: function(store, payload) {
		axios.get('/api/admin-cluster/clusters/' + payload.id + '/projects').then(response => {
			store.commit('setClusterProjects', response.data)
		}).catch(error => {
			console.log('failed get getClusterProjects')
		})
	}
}
